...
public void LoadFromJson(string json, bool silent)
{
    ...
    // YENİ ALANLAR - Backward compatibility kontrolü
    if (description.ContainsKey("Back"))
        RestoreFromString(ref Back, BackRenderer, SpriteCollection.Back, description["Back"], silent);
    
    if (description.ContainsKey("Wings"))
        RestoreFromString(ref Wings, WingsRenderer, SpriteCollection.Wings, description["Wings"], silent);
    
    RestoreFromString(ref Shield, ShieldRenderers, SpriteCollection.Shield, description["Shield"], silent);
    Expression = description["Expression"];
    Expressions = new List<Expression>();
    EquipmentTags = description.ContainsKey("EquipmentTags") && description["EquipmentTags"] != "" ? description["EquipmentTags"].Split(',').Select(Enum.Parse<EquipmentTag>).ToList() : new List<EquipmentTag>();
    ...
    // WEAPON HANDLING - Hem eski hem yeni format'ı destekle
    
    // Eski format kontrolü - PrimaryWeapon key'i varsa eski format
    ...
    // Yeni format için weapon type'a göre loading
    else
    {
        if (description.ContainsKey("PrimaryWeapon"))
        {
...
private static string SpriteToString(IEnumerable<ItemSprite> collection, Sprite sprite, Color color)
{
    ...
    if (entry == null)
    {
        // Server modunda sessizce skip et - UI gereksiz, log kirletme
        if (IsServerMode())
        {
            // Log yok - server'da sprite collection anlamsız
            return null;
        }

        // Client modunda detaylı hata
        Debug.LogError($"Can't find sprite '{sprite.name}' in SpriteCollection. Collection has {collection.Count()} items.");

        // Collection'daki sprite isimlerini logla
        var spriteNames = collection.SelectMany(i => i.Sprites).Select(s => s.name).Take(10);
        Debug.LogError($"Available sprite names (first 10): {string.Join(", ", spriteNames)}");
...
private ItemSprite RestoreFromString(ref Sprite sprite, SpriteRenderer renderer, IEnumerable<ItemSprite> collection, string serialized, bool silent = false)
{
    ...
    switch (entries.Count)
    {
        ...
        case 0:
            ...
            var collectionName = SpriteCollection?.name ?? "NULL";
            
            Debug.LogError($"🔴 SPRITE BULUNAMADI!");
            Debug.LogError($"📍 Sahne: {sceneName}");
            Debug.LogError($"🎯 Aranan ID: '{id}'");
            Debug.LogError($"🎮 Renderer: {renderer?.name} ({renderer?.gameObject?.name})");
            Debug.LogError($"📦 Collection: {collectionName} (Toplam: {collectionList.Count} item)");
            
            if (collectionList.Count > 0)
            {
                Debug.LogError($"📋 Collection'daki ilk 10 ID:");
                for (int i = 0; i < Mathf.Min(10, collectionList.Count); i++)
                {
                    Debug.LogError($"   {i}: {collectionList[i].Id}");
                }
                
                // Benzer ID'leri ara
                var similarIds = collectionList.Where(i => i.Id.Contains(id.Split('.').Last())).Take(5).ToList();
                if (similarIds.Count > 0)
                {
                    Debug.LogError($"🔍 Benzer ID'ler:");
                    similarIds.ForEach(s => Debug.LogError($"   Benzer: {s.Id}"));
                }
            }
            ...
            else throw new Exception($"Entry with id {id} not found in SpriteCollection.");
        default:
            Debug.LogWarning($"🟡 ÇOKLU SPRITE: {id} - Count: {entries.Count}");
            if (silent) return null; else throw new Exception($"Multiple entries with id {id} found in SpriteCollection.");
    }
}
    ...
    for (var i = 0; i < values.Length; i++)
    {
        ...
        switch (entries.Count)
        {
            ...
            case 0:
                ...
                Debug.LogError($"🔴 LIST SPRITE BULUNAMADI!");
                Debug.LogError($"📍 Sahne: {sceneName}");
                Debug.LogError($"🎯 Aranan ID: '{id}'");
                Debug.LogError($"📦 Collection: (Toplam: {collection.Count} item)");
                
                if (collection.Count > 0)
                {
                    Debug.LogError($"📋 Collection'daki ilk 5 ID:");
                    for (int j = 0; j < Mathf.Min(5, collection.Count); j++)
                    {
                        Debug.LogError($"   {j}: {collection[j].Id}");
                    }
                }
                
                if (silent) Debug.LogWarning("entries.Count = " + entries.Count); 
                else throw new Exception($"Entry with id {id} not found in SpriteCollection."); 
                break;
            default:
                if (silent) Debug.LogWarning("entries.Count = " + entries.Count); 
                else throw new Exception($"Multiple entries with id {id} found in SpriteCollection."); 
                break;
        }
    }
}
