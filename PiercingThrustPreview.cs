...
public class PiercingThrustPreview : BaseSkillPreview
{
    public override string SkillId => "piercing_thrust";
    
    // Executor'dan range'i al
    private float SKILL_RANGE => PiercingThrustExecutor.GetSkillRange();
    private const float LINE_WIDTH = 1.2f;
    ...
    {
        previewObject = new GameObject("PiercingThrust_Preview");
    ...
    private Sprite CreateLineSprite()
    {
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                colors[y * width + x] = new Color(0.2f, 0.6f, 1f, alpha); // Mavi renk
            }
        }
    ...
    private void UpdatePulseAnimation()
    {
        ...
        pulseTimer += Time.deltaTime * 2f; // Daha hızlı pulse
    ...
    private List<GameObject> FindTargetsInLine(GameObject caster, Vector3 origin, Vector2 direction)
    {
        ...
        if (MonsterManager.Instance != null)
        {
            ...
            foreach (var monsterBehaviour in allMonsters)
            {
                ...
                // Line üzerinde mi kontrol et (PiercingThrustExecutor'daki IsTargetOnDashPath ile aynı)
                if (IsTargetOnDashPath(origin, endPoint, monsterBehaviour.transform.position))
                {
        ...
        {
            // Fallback: MonsterManager yoksa eski yöntemi kullan
            ...
            foreach (GameObject monster in allMonsters)
            {
                ...
                if (IsTargetOnDashPath(origin, endPoint, monster.transform.position))
                {
                    Vector2 rayDirection = monster.transform.position - origin;
                    float distance = rayDirection.magnitude;
                    RaycastHit2D hit = Physics2D.Raycast(origin, rayDirection.normalized, distance, LayerMask.GetMask("Obstacles"));
    ...
    private bool IsTargetOnDashPath(Vector3 startPos, Vector3 endPos, Vector3 targetPos)
    {
        // PiercingThrustExecutor'daki aynı logic
        ...
        // Hit threshold (character width) - PiercingThrustExecutor ile aynı
        return perpendicularDistance <= 1.2f;
    }
