    ...
    */
            ...
            switch (part)
            {
                case EquipmentPart.Helmet:
                    ...
                    if (item != null && item.Tags.Contains("ShowEars")) EquipmentTags.Add(EquipmentTag.ShowEars);
                    if (item != null && item.Tags.Contains("FullHair")) EquipmentTags.Add(EquipmentTag.FullHair);
...
case EquipmentPart.MeleeWeapon2H:
    PrimaryWeapon = item?.Sprite;
    PrimaryWeaponRenderer.color = color ?? (item != null && item.Tags.Contains("Paint") ? PrimaryWeaponRenderer.color : Color.white);
    WeaponType = WeaponType.Melee2H; // Bu satırı kaldır veya koşullu yap
    if (CompositeWeapon == null) WeaponType = WeaponType.Melee2H; // Sadece ranged yoksa değiştir
    break;
                case EquipmentPart.SecondaryMelee1H:
...
case EquipmentPart.Bow:
    ...
    // WeaponType = WeaponType.Bow; // Bu satırı kaldır veya koşullu yap  
    if (PrimaryWeapon == null) WeaponType = WeaponType.Bow; // Sadece melee yoksa değiştir
    break;
case EquipmentPart.Crossbow:
    CompositeWeapon = item?.Sprites.ToList();
    // WeaponType = WeaponType.Crossbow; // Bu satırı kaldır veya koşullu yap
    if (PrimaryWeapon == null) WeaponType = WeaponType.Crossbow; // Sadece melee yoksa değiştir
    break;
                case EquipmentPart.Firearm1H:
        ...
        public void ResetEquipment()
        {
            ...
            EquipmentTags = new List<EquipmentTag>();
            Initialize();
        }
